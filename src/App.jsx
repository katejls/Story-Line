...
export default function App() {
  ...
  var cMsgs = (chatPartner && allChats[chatPartner]) ? (allChats[chatPartner].msgs || []) : [];
  var cHist = (chatPartner && allChats[chatPartner]) ? (allChats[chatPartner].hist || []) : [];
  ...
  function saveChatToStorage(storyId, name, newAllChats) {
    var updated = savedStories.map(function(s) {
      if (s.id === storyId) { return Object.assign({}, s, { allChats: newAllChats }); }
      return s;
    });
  ...
  function generateStory(scId, p, l, v, endingType, pg, lg, vg, vr, spice, sl, slg) {
    ...
    callAPI(sys, msgs, 2500).then(function(d) {
      ...
      var initHist = [
        {role: "user", content: "You are " + chatWith + " from a " + sc.label + " story, texting " + p + ". Stay in character. 1-3 sentences. Text casually. NO asterisks. Never mention AI."},
  ...
  function doReset() {
    ...
    setAllChats({}); setActiveStoryId(null); setChatPartner(null);
  }
  ...
  function sendChat() {
    ...
    var newChats = Object.assign({}, allChats);
    newChats[chatPartner] = { msgs: newMsgs, hist: cHist };
    ...
    callAPI(sys, apiMsgs, 200).then(function(d) {
      ...
      var finalChats = Object.assign({}, allChats);
      finalChats[chatPartner] = { msgs: updatedMsgs, hist: updatedHist };
      setAllChats(finalChats);
      if (activeStoryId) saveChatToStorage(activeStoryId, chatPartner, finalChats);
      setCBusy(false);
    }).catch(function() {
      setCBusy(false);
    });
        ...
        <div style={{display: "flex", gap: 0, background: "#0d0d12", borderRadius: 14, overflow: "hidden", border: "1px solid #1a1a24", marginBottom: 16}}>
          ...
          <button style={nb(false)} onClick={function() {
              ...
              if (!allChats[cp] || !allChats[cp].hist || allChats[cp].hist.length === 0) {
                ...
                var nc = Object.assign({}, allChats);
                nc[cp] = { msgs: [], hist: [{role: "user", content: "You are " + cp + " from a " + (sc2 && sc2.label) + " story, texting " + pName + ". Stay in character. 1-3 sentences. Text casually. NO asterisks. Never mention AI."}, {role: "assistant", content: "I understand."}] };
                setAllChats(nc);
              }
              setScreen("chat");
          ...
          <div style={{maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", gap: 8}}>
            <button style={bt(false)} onClick={function() {
              ...
              if (!allChats[lName] || !allChats[lName].hist || allChats[lName].hist.length === 0) {
                var sc2 = SCENARIOS.find(function(s2) { return s2.id === scenario; });
                var nc = Object.assign({}, allChats);
                nc[lName] = { msgs: [], hist: [{role: "user", content: "You are " + lName + " from a " + (sc2 && sc2.label) + " story, texting " + pName + ". Stay in character. 1-3 sentences. Text casually. NO asterisks. Never mention AI."}, {role: "assistant", content: "I understand."}] };
                setAllChats(nc);
              }
              setScreen("chat");
            }}>{lName} wants to talk to you...</button>
            {ending === "betrayal" && slName && <button style={Object.assign({}, b2, {borderColor: "#2e86de40", color: "#2e86de"})} onClick={function() {
              setChatPartner(slName);
              if (!allChats[slName] || !allChats[slName].hist || allChats[slName].hist.length === 0) {
                var sc2 = SCENARIOS.find(function(s2) { return s2.id === scenario; });
                var nc = Object.assign({}, allChats);
                nc[slName] = { msgs: [], hist: [{role: "user", content: "You are " + slName + " from a " + (sc2 && sc2.label) + " story, texting " + pName + ". You have feelings for them. Stay in character. 1-3 sentences. Text casually. NO asterisks. Never mention AI."}, {role: "assistant", content: "I understand."}] };
                setAllChats(nc);
              }
              setScreen("chat");
