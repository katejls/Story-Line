...
export default function App() {
  var _screen = useState("splash");
  var screen = _screen[0]; var setScreen = _screen[1];
  var _scenario = useState(null);
  var scenario = _scenario[0]; var setScenario = _scenario[1];
  var _pName = useState("");
  var pName = _pName[0]; var setPName = _pName[1];
  var _lName = useState("");
  var lName = _lName[0]; var setLName = _lName[1];
  var _nMode = useState(null);
  var nMode = _nMode[0]; var setNMode = _nMode[1];
  var _lMode = useState(null);
  var lMode = _lMode[0]; var setLMode = _lMode[1];

  var _chapters = useState([]);
  var chapters = _chapters[0]; var setChapters = _chapters[1];
  var _curText = useState("");
  var curText = _curText[0]; var setCurText = _curText[1];
  var _shown = useState("");
  var shown = _shown[0]; var setShown = _shown[1];
  var _chNum = useState(0);
  var chNum = _chNum[0]; var setChNum = _chNum[1];
  var _hist = useState([]);
  var hist = _hist[0]; var setHist = _hist[1];
  var _busy = useState(false);
  var busy = _busy[0]; var setBusy = _busy[1];
  var _lmsg = useState(LMSGS[0]);
  var lmsg = _lmsg[0]; var setLmsg = _lmsg[1];

  var _cMsgs = useState([]);
  var cMsgs = _cMsgs[0]; var setCMsgs = _cMsgs[1];
  var _cIn = useState("");
  var cIn = _cIn[0]; var setCIn = _cIn[1];
  var _cBusy = useState(false);
  var cBusy = _cBusy[0]; var setCBusy = _cBusy[1];
  var _cHist = useState([]);
  var cHist = _cHist[0]; var setCHist = _cHist[1];
  var _err = useState("");
  ...
  useEffect(function() {
    stateRef.current = { scenario: scenario, pName: pName, lName: lName, chNum: chNum, curText: curText, hist: hist };
  });
  ...
  useEffect(function() {
    ...
    twRef.current = setInterval(function() {
      i++;
      setShown(curText.slice(0, i));
      if (i >= curText.length) clearInterval(twRef.current);
    }, 10);
  ...
  function gen(isNew, scId, p, l, prevHist, prevChNum, prevCurText) {
    setBusy(true);
    setErr("");
    ...
    var uMsg = isNew
      ? "Write Chapter 1. Drop me into a pivotal moment. Make me feel " + l + " presence immediately."
    ...
    fetch("https://api.anthropic.com/v1/messages", {
      ...
      if (isNew) { setChapters([]); setChNum(1); }
        ...
        setChapters(function(prev) { return prev.concat([{text: prevCurText, num: prevChNum}]); });
        setChNum(num);
      }
      setCurText(txt);
      setScreen("story");
  ...
  function fmt(text) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (ln.indexOf("**") === 0 && ln.lastIndexOf("**") === ln.length - 2) {
        result.push(React.createElement("h3", {key: i, style: {fontSize: 19, fontWeight: 600, color: accent, margin: "18px 0 12px", lineHeight: 1.3}}, ln.split("**").join("")));
      } else if (ln === "---") {
        result.push(React.createElement("div", {key: i, style: {height: 1, background: "linear-gradient(90deg,transparent,#1a1a24,transparent)", margin: "20px 0"}}));
      } else if (!ln.trim()) {
        result.push(React.createElement("br", {key: i}));
      } else {
        result.push(React.createElement("p", {key: i, style: {marginBottom: 6}}, ln));
      }
    }
    return result;
  }

  function cd(isSel, clr) {
    return {padding: "16px", borderRadius: 16, border: isSel ? "1.5px solid " + clr : "1.5px solid #18181f", background: isSel ? clr + "10" : "#0d0d14", cursor: "pointer", transition: "all 0.3s", display: "flex", gap: 14, alignItems: "center"};
  }
  function bt(off) {
    return {width: "100%", padding: "16px", borderRadius: 14, border: "none", background: off ? "#1a1a24" : "linear-gradient(135deg," + accent + "," + accent + "bb)", color: off ? "#4a4540" : "#fff", fontSize: 15, fontWeight: 600, fontFamily: FONT, cursor: off ? "default" : "pointer", letterSpacing: 1, boxShadow: off ? "none" : "0 4px 30px " + accent + "30"};
  }
  ...
  function nb(a) {
    return {flex: 1, padding: "12px", background: a ? accent + "15" : "transparent", color: a ? accent : "#4a4540", border: "none", fontSize: 12, fontFamily: MONO, letterSpacing: 1, cursor: "pointer", borderBottom: a ? "2px solid " + accent : "2px solid transparent"};
  }
  ...
  if (screen === "scenario") return (
    <div style={W}><div style={G}/>
      <div style={C}>
        ...
        <p style={{fontSize: 13, color: "#5a544a", marginBottom: 16, fontStyle: "italic"}}>Tap the play button to jump in instantly.</p>
        <div style={{display: "flex", flexDirection: "column", gap: 10}}>
          {SCENARIOS.map(function(s) { return (
            <div key={s.id} style={cd(scenario === s.id, s.color)} onClick={function() { setScenario(s.id); }}>
              ...
              <div onClick={function(e) { e.stopPropagation(); quickStart(s.id); }} style={{width: 40, height: 40, borderRadius: "50%", background: s.color + "25", border: "1.5px solid " + s.color + "50", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, fontSize: 16, color: s.color}}>&#9654;</div>
            </div>
          ); })}
        </div>
        {scenario && (
          <div style={{marginTop: 20, display: "flex", flexDirection: "column", gap: 8}}>
            <button style={bt(false)} onClick={function() { quickStart(scenario); }}>Quick Start - Random Names</button>
            <button style={b2} onClick={function() { setScreen("nameSetup"); }}>Customize Names</button>
          </div>
        )}
      </div>
      <style>{CSS_TEXT}</style>
    ...
    <div style={W}><div style={G}/>
      <div style={C}>
        ...
        <div style={{marginBottom: 24}}>
          ...
          <div style={cd(nMode === "custom", accent)} onClick={function() { setNMode("custom"); setPName(""); }}><div style={{fontSize: 22, width: 36, textAlign: "center"}}>&#9997;&#65039;</div><div style={{fontSize: 14, fontWeight: 600}}>Type your name</div></div>
          ...
          <div style={Object.assign({}, cd(nMode === "random", accent), {marginTop: 8})} onClick={function() { setNMode("random"); setPName(rName()); }}><div style={{fontSize: 22, width: 36, textAlign: "center"}}>&#127922;</div><div style={{fontSize: 14, fontWeight: 600}}>Random name</div></div>
          {nMode === "random" && <div style={{display: "flex", alignItems: "center", gap: 8, marginTop: 8}}><span style={pl}>{pName}</span><button style={Object.assign({}, b2, {width: "auto", padding: "6px 14px"})} onClick={function() { setPName(rName()); }}>&#127922;</button></div>}
        </div>
        ...
        <div>
          ...
          <div style={cd(lMode === "custom", accent)} onClick={function() { setLMode("custom"); setLName(""); }}><div style={{fontSize: 22, width: 36, textAlign: "center"}}>&#9997;&#65039;</div><div style={{fontSize: 14, fontWeight: 600}}>Type their name</div></div>
          {lMode === "custom" && <input style={Object.assign({}, inp, {marginTop: 8})} placeholder="Their name..." value={lName} onChange={function(e) { setLName(e.target.value); }}/>}
          <div style={Object.assign({}, cd(lMode === "random", accent), {marginTop: 8})} onClick={function() { setLMode("random"); setLName(rName()); }}><div style={{fontSize: 22, width: 36, textAlign: "center"}}>&#127922;</div><div style={{fontSize: 14, fontWeight: 600}}>Random name</div></div>
          {lMode === "random" && <div style={{display: "flex", alignItems: "center", gap: 8, marginTop: 8}}><span style={pl}>{lName}</span><button style={Object.assign({}, b2, {width: "auto", padding: "6px 14px"})} onClick={function() { setLName(rName()); }}>&#127922;</button></div>}
        </div>
  ...
  if (screen === "story") return (
    <div style={W}><div style={G}/>
      <div style={Object.assign({}, C, {paddingBottom: 100})}>
        ...
        <div ref={sRef} style={{maxHeight: "58vh", overflowY: "auto", padding: "20px", background: "#09090f", borderRadius: 16, border: "1px solid #141420", lineHeight: 1.9, fontSize: 15.5, whiteSpace: "pre-wrap"}}>
          {chapters.map(function(ch, i) { return <div key={i} style={{marginBottom: 20, opacity: 0.7}}>{fmt(ch.text)}<div style={{height: 1, background: "linear-gradient(90deg,transparent,#1a1a24,transparent)", margin: "20px 0"}}/></div>; })}
          {fmt(shown)}
        </div>
